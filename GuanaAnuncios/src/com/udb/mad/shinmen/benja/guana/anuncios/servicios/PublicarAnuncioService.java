...
public class PublicarAnuncioService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		JSONObject respuesta = postRequest("http://guananuncio.madxdesign.com/index.php/anuncio/save", parametros);
		
		String status = null;
		try {
			status = respuesta.getString("estado");
		} catch (JSONException e) {
			// TODO Loguear apropiadamente
			e.printStackTrace();
		}
		if (status.equals("1")) {
			notificar();
	...
	private void notificar(){
		...
		.setContentText("Anuncio Publicado");
